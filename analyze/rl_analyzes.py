...
def overall_performance(directory):
    ...
    for j, d in enumerate(natsorted(os.listdir(directory))):
        ...
        dfs_combined = dfs_combined[d].groupby('aligned_timestamp').agg({'throughput': 'mean'}).reset_index()
        dfs_combined = bucket_and_average(dfs_combined[d], ['throughput'], num_buckets=780)
        ax1.plot(dfs_combined[d]['aligned_timestamp'], dfs_combined[d]['throughput'], label=d + ' Traffic', color=colors[j], linewidth=3)
