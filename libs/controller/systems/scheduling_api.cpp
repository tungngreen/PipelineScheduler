...
void Controller::ScheduleSingleTask(const std::string &msg) {
    TaskDesc request;
    ...
    crossDeviceWorkloadDistributor(task, task->tk_slo / 2);
    shiftModelToEdge(task->tk_pipelineModels, task->tk_pipelineModels.front(),
                     task->tk_slo, task->tk_pipelineModels.front()->device);
    for (auto &model: task->tk_pipelineModels) {
        if (model->name.find("_") == std::string::npos)
            model->name = taskName + "_" + model->name;
    }
    estimateModelTiming(task->tk_pipelineModels.front(), 0);
    ...
    for (auto &model: task->tk_pipelineModels) {
        ...
        estimatePipelineLatency(model, model->expectedStart2HereLatency);
    }
