...
FCRLAgent::FCRLAgent(std::string& cont_name, uint state_size, uint resolution_size, uint max_batch,  uint scaling_size,
    ...
    model = std::make_shared<MultiObjectiveMultiPolicyNet>(state_size, resolution_size, max_batch, scaling_size);
...
void FCRLAgent::update() {
    ...
    try {
        ...
        // TODO: fix the training here with using all vector shaped rewards
        T policy_loss_2 = torch::exp(-torch::tensor(experiences.get_rewards()[0]).to(precision));
...
void FCRLAgent::federatedUpdate(const double loss) {
    FlData request;
...
void FCRLAgent::federatedUpdateCallback(FlData &response) {
...
void FCRLAgent::rewardCallback(double throughput, double drops, double latency_penalty, double oversize_penalty) {
    if (update_steps > 0 ) {
...
void FCRLAgent::setState(double curr_resolution, double curr_batch, double curr_scaling,  double arrival,
...
T FCRLAgent::computeCumuRewards() const {
    // TODO: fix the training here with using all vector shaped rewards
    std::vector<double> discounted_rewards, rewards = experiences.get_rewards()[0];
...
T FCRLAgent::computeGae() const {
    // TODO: fix the training here with using all vector shaped rewards
    std::vector<double> advantages, rewards = experiences.get_rewards()[0];
...
std::tuple<int, int, int> FCRLAgent::runStep() {
    Stopwatch sw;
    sw.start();
    selectAction();
...
FCRLServer::FCRLServer(std::string run_name, uint state_size, torch::Dtype precision)
    ...
    re = std::mt19937(rd());
    model = std::make_shared<MultiObjectiveMultiPolicyNet>(state_size, 1, 1, 1);
    std::string model_save = path + "/latest_model.pt";
    ...
    federated_clients = {};
    run = true;
    std::thread t(&FCRLServer::proceed, this);
    t.detach();
}

bool FCRLServer::addClient(FlData &request, std::shared_ptr<ControlCommands::Stub> stub, CompletionQueue *cq) {
    std::istringstream iss(request.network());
    federated_clients.push_back({request,
                                 std::make_shared<MultiObjectiveMultiPolicyNet>(request.state_size(), request.resolution_size(),
...
void FCRLServer::returnFLModel(ClientModel &client) {
    FlData request;
    std::ostringstream oss;
